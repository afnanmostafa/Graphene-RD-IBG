function [finalfile] = gen_RDIBG_topDown_func(lengths,widths,stackings,layers,bond_density)
%   RD-IBG generator from 2D diamond
%   I: length, width, stacking(aa/ab), layers
...
for h=1:length(lengths)
    for g=1:length(widths)
        for idx=1:length(stackings)
            ...
            [file,~,~,~] = generate_graphene(length_sheet,width_sheet,stackings(idx),layers);
            ...
            script = 'in_%s_%dx%d_%.2fBD.lmp';
            fid2 = sprintf(script, stackings(idx),length_sheet,width_sheet,final_bd);
            fid2 = fopen(fid2, 'w');
